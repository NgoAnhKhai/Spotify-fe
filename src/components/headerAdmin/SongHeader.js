...
export default function SongHeader() {
  ...
  const handleSearchKeyPress = async (event) => {
    ...
    if (event.key === "Enter" && query.trim() !== "") {
      try {
        const results = await fetchFindSongByTitle(query);

        console.log("result:", results.song);
        if (results) {
          setSongTitle(results.song);
          setSnackbarMessage("song found successfully!");
          setSnackbarSeverity("success");
          setSnackbarOpen(true);
        } else {
          setSnackbarMessage("No song found.");
          setSnackbarSeverity("error");
          setSnackbarOpen(true);
        }
      } catch (error) {
        console.error("Error searching song:", error);
        setSnackbarMessage("Error while searching for song.");
        setSnackbarSeverity("error");
        setSnackbarOpen(true);
      }
    }
