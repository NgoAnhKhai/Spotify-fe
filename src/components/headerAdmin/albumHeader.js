...
export default function AlbumHeader() {
  ...
  const { setAlbumTitle } = useContext(AlbumContext);
  ...
  const handleSearchKeyPress = async (event) => {
    ...
    if (event.key === "Enter" && query.trim() !== "") {
      try {
        const results = await fetchFindAlbumByTitle(query);

        console.log("result:", results.albums);
        if (results) {
          setAlbumTitle(results.albums);
          setSnackbarMessage("album found successfully!");
          setSnackbarSeverity("success");
          setSnackbarOpen(true);
        } else {
          setSnackbarMessage("No album found.");
          setSnackbarSeverity("error");
          setSnackbarOpen(true);
        }
      } catch (error) {
        console.error("Error searching album:", error);
        setSnackbarMessage("Error while searching for album.");
        setSnackbarSeverity("error");
        setSnackbarOpen(true);
      }
    }
