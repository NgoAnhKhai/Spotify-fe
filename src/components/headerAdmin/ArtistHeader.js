...
export default function ArtistHeader() {
  ...
  const { setArtistName } = useContext(ArtistContext);
  ...
  const handleSearchKeyPress = async (event) => {
    ...
    if (event.key === "Enter" && query.trim() !== "") {
      try {
        const results = await fetchFindArtistByName(query);

        console.log("result:", results.artists);
        if (results) {
          setArtistName(results.artists);
          setSnackbarMessage("Artist found successfully!");
          setSnackbarSeverity("success");
          setSnackbarOpen(true);
        } else {
          setSnackbarMessage("No Artist found.");
          setSnackbarSeverity("error");
          setSnackbarOpen(true);
        }
      } catch (error) {
        console.error("Error searching Artist:", error);
        setSnackbarMessage("Error while searching for Artist.");
        setSnackbarSeverity("error");
        setSnackbarOpen(true);
      }
    }
